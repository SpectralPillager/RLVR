...
# Merged version: block.py, att.py, ffn.py, model.py, rwkvop.py
########################################################################################################
...
if 'x070' in os.environ.get("RWKV_MY_TESTING", ""):
    ...
    class WindBackstepping(torch.autograd.Function):
        ...
        def forward(ctx, w,q,k,v,z,b):
            ...
            assert all(i.dtype == dtype for i in [q, k, v, z, b])
    ...
    def RUN_CUDA_RWKV7g(q,w,k,v,a,b, HEAD_SIZE=64):
        ...
        C = HEAD_SIZE
        H = HC // C
        dtypes = {q.dtype, w.dtype, k.dtype, v.dtype, a.dtype, b.dtype}
        assert dtypes.issubset({torch.float16, torch.bfloat16, torch.float32}), f"unexpected dtypes: {dtypes}"
        if torch.float32 in dtypes or torch.float16 in dtypes:
    ...
    if os.environ.get("RWKV_TRAIN_TYPE") in ['state', 'fullstate']:
        ...
        class WKV7StateFunction(torch.autograd.Function):
            ...
            def forward(ctx, w, q, k, v, z, b, s0):
                ...
                dtype = w.dtype
                assert dtype in (torch.bfloat16, torch.float32)
                assert all(i.dtype == dtype for i in [q, k, v, z, b, s0])
                w, q, k, v, z, b = [i.contiguous() for i in [w, q, k, v, z, b]]
                s0 = s0.contiguous()
            ...
            def backward(ctx, dy):
                ...
                dw, dq, dk, dv, dz, db = [torch.empty_like(x) for x in [w, q, k, v, z, b]]
                ds0 = torch.empty(B, H, C, C, dtype=w.dtype, device=w.device)
        ...
        def RUN_RWKV7_STATE(r, k, v, w, a, b, s, HEAD_SIZE=64):
            ...
            dtypes = {r_4d.dtype, k_4d.dtype, v_4d.dtype, w_4d.dtype, a_4d.dtype, b_4d.dtype, s_3d.dtype}
            assert dtypes.issubset({torch.float16, torch.bfloat16, torch.float32}), f"unexpected dtypes: {dtypes}"
            # Under DataParallel + autocast, some shards may produce fp16 activations.
            # The custom state kernel only supports bf16/fp32, so upcast fp16 shards.
            # Prefer fp32 when any input is fp32/fp16 to preserve stability.
            if torch.float32 in dtypes or torch.float16 in dtypes:
                compute_dtype = torch.float32
            else:
                compute_dtype = torch.bfloat16
            if r_4d.dtype != compute_dtype:
                r_4d = r_4d.to(compute_dtype)
            if k_4d.dtype != compute_dtype:
                k_4d = k_4d.to(compute_dtype)
            if v_4d.dtype != compute_dtype:
                v_4d = v_4d.to(compute_dtype)
            if w_4d.dtype != compute_dtype:
                w_4d = w_4d.to(compute_dtype)
            if a_4d.dtype != compute_dtype:
                a_4d = a_4d.to(compute_dtype)
            if b_4d.dtype != compute_dtype:
                b_4d = b_4d.to(compute_dtype)
            if s_3d.dtype != compute_dtype:
                s_3d = s_3d.to(compute_dtype)
...
class RWKV_Tmix_x070(nn.Module):
    ...
    def addcmul(self, x, xx):
        if fused_addcmul_rwkv7 is not None:
            return self.fused_addcmul(x, xx)
        return self.torch_addcmul(x, xx)
    ...
    def forward(self, x, v_first, attention_mask=None):
        ...
        xr, xw, xk, xv, xa, xg = self.addcmul(x, xx)
...
class RWKV7(nn.Module):
    ...
    def forward_normal(self, input_ids, inputs_embeds=None, attention_mask=None, **kwargs):
        args = self.args
        last_token_only = bool(kwargs.pop("last_token_only", False))
        B, T = input_ids.size()
        assert T <= args.ctx_len, "Cannot forward, model ctx_len is exhausted."

        use_fp32_forward = os.environ.get("RWKV_MODEL_FP32_FORWARD", "1") == "1"

        def _run_blocks(x, v_first):
            use_cp = (args.grad_cp == 1) and self.training and torch.is_grad_enabled()
            for block in self.blocks:
                if use_cp:
                    if args.train_type in ('state', 'fullstate') or args.peft !='none':
        ...
        if use_fp32_forward:
            with torch.autocast(device_type=input_ids.device.type, enabled=False):
                ...
                x, v_first = _run_blocks(x, v_first)
                x = self.ln_out(x)
                if last_token_only:
                    x = x[:, -1, :]
                x = self.head(x)
